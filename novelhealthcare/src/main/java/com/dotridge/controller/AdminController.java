...
public class AdminController
{
	...
	public String viewAllAdmins(Model model)
	{
		try
		{
			List<AdminBeanTest> adminBeansList = adminService.getAllAdmins();
	...
	public String addAdmin(@ModelAttribute("adminBean") AdminBeanTest adminBean, Model model)
	{
		System.out.println(adminBean.toString());
		AdminBeanTest adminBean2 = adminService.createAdmin(adminBean);
		if(adminBean2.getAdminId() > 0)
		{
				List<AdminBeanTest> uiAdminsList = adminService.getAllAdmins();
	...
	public String editAdmin(HttpServletRequest request, Model model)
	{
		...
		AdminBeanTest adminBean = adminService.getAdminById(adminId);
	...
	public String updateAdmin(@ModelAttribute("adminBean")AdminBeanTest adminBean, Model model)
	{
		if(adminBean != null && adminBean.getAdminId() > 0)
		{
			adminService.updateAdmin(adminBean);
			List<AdminBeanTest> adminBeans = adminService.getAllAdmins();
	...
	public String searchAdmins(HttpServletRequest request, Model model)
	{
		...
		if(searchKey != null && !searchKey.isEmpty() && searchValue != null && !searchValue.isEmpty())
		{
			...
			{
				List<AdminBeanTest> adminssList = adminService.getAdminByName(searchValue);
			...
			{
				List<AdminBeanTest> adminsList = adminService.getAdminByEmail(searchValue);
			...
			{
				List<AdminBeanTest> adminsList = adminService.getAdminByPhone(Long.parseLong(searchValue));
				if(adminsList != null && !adminsList.isEmpty())
				{
					model.addAttribute("uiAdminsList",adminsList);
					return "getUsersBoard";
			...
			{
				List<AdminBeanTest> adminsList = adminService.getAdminByStatus(Boolean.valueOf(searchValue));
				if(adminsList != null && !adminsList.isEmpty())
				{
					model.addAttribute("uiAdminsList",adminsList);
					return "getUsersBoard";
	...
	public String getAllAdminsByPagening(HttpServletRequest request, Model model)
	{
		...
		List<AdminBeanTest> adminsList = adminService.getAllAdminsByPagining(currentPageInt, noOfRecordsPerPage);
		
		List<AdminBeanTest> uiAdminList = adminService.getAllAdmins();
